function [yabba,dabba,doo] = synthlay_grad1( basenm, x01,y01,z01,x02,y02,z02,x03,y03,z03,x04,y04,z04,x05,y05,z05,x06,y06,z06,x07,y07,z07,x08,y08,z08,x09,y09,z09,x010,y010,z010,x011,y011,z011,dipangle1,dipangle2,dipangle3,dipangle4,dipangle5,dipangle6,dipangle7,dipangle8,dipangle9,dipangle10,dipangle11,strikeangle1,strikeangle2,strikeangle3,strikeangle4,strikeangle5,strikeangle6,strikeangle7,strikeangle8,strikeangle9,strikeangle10,strikeangle11,fault_thickness1,fault_thickness2,fault_thickness3,fault_thickness4,fault_thickness5,fault_thickness6,fault_thickness7,fault_thickness8,fault_thickness9,fault_thickness10,fault_thickness11,c1,c2,c3,ts,step,numg,gfrac1,gfrac2,gfrac3,gfrac4)
%SGR 7/2012
...
thickness2=zeros(maxnumlay, length(xc));
...
for i=1:length(xc);
    ...
    g=1;
    for a=1:tic-1
        g=sum(thickness2(:,i)~=0)+1;
        if C(a,i)==c2
            if g==1
                cw=ft(g,1)/w_mult; w=(ft(g,1)-cw)/2;  d=ft(g,1)-thickness(a,i);
                if d>w+cw
                    thickness2(g,i)= thickness(a,i);
                    C2(g,i)=c2;
                    continue
                end
                if d>=w
                    thickness2(g+1,i)= w;
                    C2(g+1,i)=c2;
                    thickness2(g,i)= thickness(a,i)-cw;
                    C2(g,i)=c3;
                    continue
                end
                if d<w
                    ...
                    thickness2(g+1,i)= cw;
                    C2(g+1,i)=c3;
                    thickness2(g,i)= thickness(a,i)-w+cw;
                    C2(g,i)=c2;
                    continue
                end
            else
                cw=ft(a,1)/w_mult; w=(ft(a,1)-cw)/2;
                thickness2(g+2,i)= w;
                C2(g+2,i)=c2;
                thickness2(g+1,i)= cw;
                C2(g+1,i)=c3;
...
thickness2=round(10*thickness2)/10;
...
for j=1:length(xc)
    nlayers(j)=sum(thickness2(:,j)>0);
    layindex=find(thickness2(:,j));
    ...
    for i=1:nlayers(j)
        ...
        fprintf(lfid,'%.2f %f %.0f\n',thickness2(layindex(i),j),kb(layindex(i),j),0); %end
        %         if C2(layindex(i),j)==c2 || C2(layindex(i),j)==c3
        %             fprintf(lfid,'%.2f %f %.0f\n',thickness2(layindex(i),j),kb(layindex(i),j),1); end
        if numg==1
            fprintf(lfid,'%.2f\n',thickness2(layindex(i),j));
        end
        if numg==2
            if C2(layindex(i),j)==c1;
                fprintf(lfid,'%.2f %.2f\n',(thickness2(layindex(i),j)*gfrac1),(thickness2(layindex(i),j)*gfrac2)); end %take gfrac 1 and 3 as the smaller grain size: proportion 1
            if C2(layindex(i),j)==c2 || C2(layindex(i),j)==c3
                fprintf(lfid,'%.2f %.2f\n',(thickness2(layindex(i),j)*gfrac3),(thickness2(layindex(i),j)*gfrac4)); end
        end
