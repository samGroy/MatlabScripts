function product = synthlay_grad6( basenm, matrix, ts, step, numg, gfrac1, gfrac2, gfrac3, gfrac4)
%SGR 7/2012
...
% SGR 12/2013 The Cairngorn volume TODO
...
% Read child prelim file, omit boundary nodes %%%FOR cairngorns, maybe
% crop x and y limits here for Cairngorns? then anything outside of this
% would be a single layer. Then add another layer within cairngorn limits
...
%             if C(layindex(i),j)==cohesion(1) || C(layindex(i),j)==cohesion(3) || C(layindex(i),j)==cohesion(2);
...
%         end
%         if numg==2
%             if C(layindex(i),j)==cohesion(4) || C(layindex(i),j)==cohesion(3);
%                 fprintf(lfid,'%.2f %.2f\n',(thickness(layindex(i),j)*gfrac4),(thickness(layindex(i),j)*gfrac1)); end %take gfrac 1 and 3 as the smaller grain size: proportion 1
...
% 
        if numg==2
            if C(layindex(i),j)==cohesion(4);
                fprintf(lfid,'%.2f %.2f\n',(thickness(layindex(i),j)*gfrac4),(thickness(layindex(i),j)*gfrac2)); end %take gfrac 1 and 3 as the smaller grain size: proportion 1
            if C(layindex(i),j)==cohesion(3);
                fprintf(lfid,'%.2f %.2f\n',(thickness(layindex(i),j)*gfrac3),(thickness(layindex(i),j)*gfrac3)); end %take gfrac 1 and 3 as the smaller grain size: proportion 1
            if C(layindex(i),j)==cohesion(2);
                fprintf(lfid,'%.2f %.2f\n',(thickness(layindex(i),j)*gfrac2),(thickness(layindex(i),j)*gfrac4)); end %take gfrac 1 and 3 as the smaller grain size: proportion 1
            if C(layindex(i),j)==cohesion(1);
                fprintf(lfid,'%.2f %.2f\n',(thickness(layindex(i),j)*gfrac1),(thickness(layindex(i),j)*gfrac5)); end %take gfrac 1 and 3 as the smaller grain size: proportion 1
        end
