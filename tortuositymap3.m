function [nodedat,h]=tortuositymap3(basenm,ts, nwavelength)
...
qfid=fopen(filenm,'r');
if netfid==0, error(['Unable to open flux file']);end
%nd(:,3)=nd(:,3)+5;
...
for i=1:length(netdat);
    ...
    while a<=nwavelength
        ...
        if a==round(nwavelength/2)-1, up=b; end
    ...
    f1=find(netdat==bb);
    f2=fluxdat(f1);
    f3=fluxdat(netdat(bb));
    f4=zeros(length(f1)); f4=f3-f2; f4=f4';
    if find(f4==min(f4))~=find(f1==up); continue, end
    if fluxdat(bb)
    end
