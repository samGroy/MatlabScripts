function [id,steparc2,nodedat,netdat]=tortuosityprofile5(basenm,ts,wavelength,x,y)
...
while front<=length(id(:,1))
    ...
    [e,front]=min(dist2(g+1:end,2)); front=front+g;
...
%     front=front+back; %find next node that hs the best fit distance from the current node
    ...
    steparc(g,1)=sqrt(dx2+dy2); %Euclidean length between back and front points
    steparc(g,2)=id(front,3)-id(back,3); %Arc length downstream
    steparc(g,3)=1-steparc(g,1)/steparc(g,2); % Tortuosity calculation
    steparc(g,4)=nodedat(id(g,1),1); steparc(g,5)=nodedat(id(g,1),2); % X and Y coords
    steparc(g,6)=id(g-1,1); % Get current node id
