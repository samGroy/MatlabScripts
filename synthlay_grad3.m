...
% maximum number of possible layers including 5 sublayers in 11 faults and the spaces
% between
maxnumlay=67;
...
% bdfeca : the sublayer transitions from top to bottom, lord knows why I
% chose to do this way.
asymmetry=0;
% fault is symmetric, fault core is centered.
if asymmetry==0
    intnode=length(xc);
...
% If fault is asymmetric, fault core is displaced to one side, the 'a' side.
% bdfeca
if asymmetry==1
    intnode=length(xc);
    zlt1a=((ortho1(1)*(xc-p01(1))+ortho1(2)*(yc-p01(2)))/ortho1(3))+z01;
    zlt1b=zlt1a+fthick1;
    zlt1c=zlt1a;
    zlt1d=zlt1a+2*fthick1/3;
    zlt1e=zlt1a;
    zlt1f=zlt1a+fthick1/3;
...
for j=1:length(xc)
    ...
    for i=1:nlayers(j)
        ...
        if numg==2
            ...
            if C(layindex(i),j)==c2 || C(layindex(i),j)==c3 || C(layindex(i),j)==c4
