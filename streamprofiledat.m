function c=streamprofiledat(basenm,ts,wavelength1,wavelength2,x,y)
...
title(['stream path, ',num2str(wavelength1),' m, ',num2str(tm),' years']);
...
k=scatter3(nodedat(id(:,1),1),nodedat(id(:,1),2),zdat(id(:,1)),10,steparc(:,3),'filled'); 
...
view([0 90]);
% h=plot3(nodedat(id(:,1),1),nodedat(id(:,1),2),zdat(id(:,1)),'b','LineWidth',10); 
%k=scatter3(nodedat(id2(:,1),1),nodedat(id2(:,1),2),zdat(id2(:,1)),(empwidth(id2(:,1))+10),steparc2(:,3),'filled'); 
k=scatter3(nodedat(id2(:,1),1),nodedat(id2(:,1),2),zdat(id2(:,1)),10,steparc2(:,3),'filled'); 
colormap([gray(64);hot(64)])
% Initially, both CDatas are equal to Z.
m = 64; % 64-elements is each colormap
...
title(['tortuosity, ',num2str(wavelength1),' m wavelength']); 
