...
maxradius=1e5; radius=900; iniradius=radius; % measure correlation to maxradius advancing by iniradius
...
cor=zeros(length(angle),maxradius/radius); % initialize raw correlogram matrix
...
for j=1:maxradius/iniradius % Step through radii
    radius=iniradius*j; % Advance radius length per iteration
    for i=1:length(angle) % Step through angles, 5 degree intervals
        xrad=cos(angle(i))*radius; % Find adjacent length (x component)
        yrad=sin(angle(i))*radius; % Find opposite length (y component)
        dx=(xx+xrad)-x(:);
        dy=(yy+yrad)-y(:);
        dist=sqrt(dx.^2+dy.^2);
...
spoke=mean_norm_cor(:,end)*maxradius;
...
% figure;
hold on;
plot3(e_x,e_y,n,'g'); %need to figure out how to convert degrees to meters for this special case. Ellipse points are in degrees, coordinates are in meters.
