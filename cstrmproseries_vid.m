function [m,distance,height,xp,yp,sp,ap,knick_d,knick_h]=cstrmproseries_vid( basenm, ts, x, y, skippy )
...
for i=1:ts
  ...
  if nargin==4
      distance(:,i)=dist;
      height(:,i)=h;
      [knick_d(i),knick_h(i)]=knickpt_tracker(dist,h,height(:,1));
  else
      distance=dist;
      height=h;
  end
  ...
  plot( dist, h, 'b', 'lineWidth', 1 )
  hold on
  %   plot(knick_d,knick_h,'ok')
%   daspect([1 1 1]);
  m(i)=getframe;
