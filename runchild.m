function product=runchild(basenm,paramfile,sequence)
...
%         end
% end
    if nargin==2 || i==1 %A new mesh is generated for each model unless a previous model run is used for initial properties.
        in1=cchangeparam(in1,'OPTREADINPUT',10);
    ...
    if paramfile(24,i) == 1 %if you requested layers,
        if nargin==2 || i==1 %if each model run is independent,
        ...
        else % if models are sequential and initial values are taken from previous run, any steps after 1 if nargin=3 go here
            ...
            oldts=ts;
            %         oldstep=step;
            paramfile(100,i)=paramfile(59,i-1)*ts;
    ...
    if nargin==2 % organize files by model run number
