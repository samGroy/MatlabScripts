...
radius=1e5; radwindow=4000; radstep=900; % measure correlation to radius advancing by radstep
...
% [val1,ind]=max(chord(:,2)); %Use this one if you're normalizing cor values.
if ind <= 18
    val2=chord(ind+18,2);
else
    val2=chord(ind-18,2);
...
% Calc aspect ratio, non-norm version
...
%     val2=chord(ind-18,2);
% end
% % Calc aspect ratio, norm version
...
% figure;
hold on;
plot3(e_x,e_y,n,'r','LineWidth',1); %need to figure out how to convert degrees to meters for this special case. Ellipse points are in degrees, coordinates are in meters.
