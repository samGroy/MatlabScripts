...
% Get average node spacing so you can figure out a proper smoothing
% distance
% cif=creadinfile([basenm '.in'],1);
% spacing=str2num(cif{26,1});
% smoothfactor=3*spacing; %current arbitrary value
% 
% for i=1:nn
%     if xyz(i,2)+10000
%     dx=xyz(:,1)-xyz(i,1);
%     dy=xyz(:,2)-(xyz(i,2)+10000;
...
ridgept=10000+ridgept; %land on max upslope
ynorm1=(ridgept-xyz(:,2))/(ridgept-min(xyz(:,2)));
ynorm2=1-(xyz(:,2)-max(xyz(:,2)))/(ridgept-max(xyz(:,2)));
for i=1:length(xyz(:,2))
    if xyz(i,2)<ridgept
        p(i)=-(ridgeht/100000)*ynorm1(i).^.5+(ridgeht/100000); %SGR changed from .008 on 11-21-2013
    else
        p(i)=-(ridgeht/250000)*ynorm2(i).^.9+(ridgeht/100000);
    end
end
% tri=delaunay(xyz(:,1),xyz(:,2));
% trisurf(tri, xyz(:,1), xyz(:,2), xyz(:,3), p)
...
tri=delaunay(xyz(:,1),xyz(:,2));
h=trisurf(tri,xyz(:,1),xyz(:,2),xyz(:,3),p);
fclose (netfid);
fclose(trifid);
fclose(sfid);