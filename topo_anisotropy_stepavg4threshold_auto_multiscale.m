...
window=0;
radius=200; radwindow=4000; radstep=50; % measure correlation to radius advancing by radstep
    ...
    if window==1 % Window switch
        ...
        for j=1:radius/radstep % Step through radii, ALL AVERAGING
            ...
            for i=1:length(angle) % Step through angles, 5 degree intervals
                ...
                yrad=sin(angle(i))*rad; % Find opposite length (y component)
                if xx+xrad<0
                    dx=(xx+xrad+max(x))-x(:); % if radius goes lower than x minimum
                elseif xx+xrad>max(x)
                    dx=(xx+xrad-max(x))-x(:); % if radius goes above x maximum
                else
                    dx=(xx+xrad)-x(:); % if radius is in domain
                end
                if yy+yrad<0
                    dy=abs(yy+yrad)-y(:);
                elseif yy+yrad>max(y)
                    dy=(2*max(y)-yy+yrad)-y(:);
                else
                    dy=(yy+yrad)-y(:);
                end
                dist=sqrt(dx.^2+dy.^2);
